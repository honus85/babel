...
//    http://docs.oracle.com/javase/1.5.0/docs/guide/jdbc/getstart/mapping.html
                    ...
                    while (true) {
                        if (isResultSet) {
                            ...
                            for (int col = 0; col < nCols; col++) {
                                ...
                                Class clazz = (Class) typeMap.get(dataTypes[col]);
                                if (clazz == null)
                                ...
                                for (int col = 0; col < nCols; col++) {
                                    switch (dataTypes[col]) {
                                        ...
                                        case (java.sql.Types.DOUBLE): {
                                            ...
                                            if (results.wasNull()) d = Double.NaN;
                                            data[col].add(d);
                                        }
                                        break;
                                        ...
                                        case (java.sql.Types.REAL): {
                                            ...
                                            if (results.wasNull()) f = Float.NaN;
                                            data[col].add(f);
                                        }
                                        break;
                                        case (java.sql.Types.TINYINT):
                                        case (java.sql.Types.SMALLINT): {
                                            short h = results.getShort(col + 1);
                                            if (results.wasNull()) h = Short.MIN_VALUE;
                                            data[col].add(h);
                                        }
                                        break;
                                        case (java.sql.Types.INTEGER): {
                                            int i = results.getInt(col + 1);
                                            if (results.wasNull()) i = Integer.MIN_VALUE;
                                            data[col].add(i);
                                        }
                                        break;
                                        case (java.sql.Types.BIGINT): {
                                            long l = results.getLong(col + 1);
                                            if (results.wasNull()) l = Long.MIN_VALUE;
                                            data[col].add(l);
                                        }
                                        break;
                                        case (java.sql.Types.LONGVARBINARY): {
                                            byte[] b = results.getBytes(col + 1);
                                            if (results.wasNull()) b = new byte[0];
                                            data[col].add(b);
                                        }
                                        break;
                                        ...
                                        case (java.sql.Types.LONGVARCHAR): {
                                            ...
                                            if (results.wasNull()) s = "";
                                            data[col].add(s.toCharArray());
                                        }
                                        break;
                                        case (java.sql.Types.DATE): {
                                            Date d = results.getDate(col + 1);
                                            if (results.wasNull()) d = (Date) c.NULL('d');
                                            data[col].add(d);
                                        }
                                        break;
                                        case (java.sql.Types.TIME): {
                                            Time t = results.getTime(col + 1);
                                            if (results.wasNull()) t = (Time) c.NULL('t');
                                            data[col].add(t);
                                        }
                                        break;
                                        case (java.sql.Types.TIMESTAMP): {
                                            Timestamp t = results.getTimestamp(col + 1);
                                            if (results.wasNull()) t = (Timestamp) c.NULL('p');
                                            data[col].add(t);
                                        }
                                        break;
                                        default: {
    ...
    static void sendResponse(c c, c.Dict cb, int msgType, Object r) throws IOException {
        switch (msgType) {
            case 0: {
                if (cb != null) c.ks((String) ((Object[]) cb.y)[0], (String) ((Object[]) cb.y)[1], new Object[]{1, r});
                else c.ks(new Object[]{1, r});
            }
            break;
            case 1:
                c.kr(r);
    ...
    static void sendError(c c, c.Dict cb, int msgType, String s) throws IOException {
        switch (msgType) {
            case 0: {
                if (cb != null) c.ks((String) ((Object[]) cb.y)[0], (String) ((Object[]) cb.y)[1], new Object[]{0, s});
                else c.ks(new Object[]{0, s});
            }
            break;
            case 1:
                c.ke(s);
    ...
    public static Object customTestFunction(char[] connection, char[] selectStatement){
        Logger.log("Test query was called: " + new String(selectStatement) + " with connection string " + new String(connection));
        String returnStatement =  "Test query was called: " + new String(selectStatement) + " with connection string " + new String(connection);
    ...
    public static Object help(){
        ...
        for( Method var : methods) {
            returnStatement = returnStatement + "," + var.getName();
        }
        return returnStatement;
    }
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            while (true) {
                try {
                    final c c = new c(ss, new c.IAuthenticate() {
                        public boolean authenticate(String s) {
                            Logger.log("Authenticating " + s.split(":", 2)[0]);
                            return true;
                        }
                    });
